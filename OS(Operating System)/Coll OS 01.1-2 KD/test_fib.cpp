...
INSTANTIATE_TEST_SUITE_P(
    ...
    FibControlTest,
    ::testing::Values(
        make_tuple(1, vector<long long>{1, 1}),
        make_tuple(2, vector<long long>{1, 1, 2}),
        make_tuple(3, vector<long long>{1, 1, 2, 3}),
        make_tuple(4, vector<long long>{1, 1, 2, 3, 5}),
        make_tuple(5, vector<long long>{1, 1, 2, 3, 5, 8})
    )
);
...
TEST(FibTest, FinConstrBigSize) {
    ...
    EXPECT_EQ(test_fib.GetSize(), 92);
    EXPECT_EQ(test_fib[91], 7540113804746346429);
}
